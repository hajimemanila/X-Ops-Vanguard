// ==UserScript==
// @name         Gemini Command Module (v4.9.43)
// @namespace    http://tampermonkey.net/
// @version      4.9.43
// @description  Enhanced prompt extraction logic and code newline fix
...
(function () {
    ...
    console.log("GCM v4.9.11 Loading (Strict Patch)...");

    const CONFIG = {
        targetTopMargin: 120,
        storageKey: 'gcm_starred_signatures_v1',
        doubleTapThreshold: 500,
        // ★ User Config: Obsidian Vault Name
        obsidianVault: "YOUR_VAULT_NAME", // ★ User Config: Set your Obsidian Vault Name here
        colors: {
            focusBorder: '#00D0FF',
    ...
    let lastEPressTime = 0; // Added
    let lastQPressTime = 0; // Added
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            /* Fix: #gcm-panel position and layout */
            #gcm-panel {
                position: fixed;
                top: 68px; /* Position slightly below header (approx 60px) */
                right: 16px; /* Align with standard UI right icon indentation */
                z-index: 2147483647 !important;
                display: flex;
                gap: 8px;
                flex-direction: row;
    ...
    function updateTargets() {
        // Generalize selector. Target all potential message containers.
        ...
        // Backup scan: Pick up divs with specific structure if standard selector fails
        ...
        messageList = candidates.filter(el => {
            const text = el.innerText.trim();
            // Must have meaningful length and not be system elements (sidebar, header)
    ...
    // ★ Refactor: Clean Text Extraction (Thorough removal of code newline corruption)
    function extractCleanText(element) {
        if (!element) return "";
        const target = element.querySelector('markdown-renderer') || element;

        // Exclude Gemini code block decorations (copy buttons, labels, hidden elements)
        ...
        // For user prompts, compress "3+ consecutive newlines" to "1" during rendering
        // Strict check to avoid affecting AI response Markdown structure (## headers etc)
        ...
        if (isUserMessage) {
            // Fix alternating empty lines (\n\n\n) in source code to normal newlines (\n)
            // Target only internal excess empty lines after trimming
            text = text.replace(/\n{2,}/g, '\n');
            // Example: Insert one newline before code blocks starting with // etc
            text = text.replace(/(\n)(\/\/ ==UserScript==)/, '\n\n$2');
        }
    ...
    // ★ Refactor: Logic to identify prompt corresponding to response (Introduced physical DOM scan)
    function getLinkedPrompt(currentEl) {
        // Criteria to determine if it's an AI response
        const isAI = (el) => {
            if (!el) return false;
        ...
        // Method A: Trace back in messageList array (Maintain existing spec)
        ...
        // Method B: Trace up physical DOM hierarchy (Fallback: Countermeasure for array inconsistency)
        // Need to trace parent of AI response container (and its siblings), using while loop
        let searchEl = currentEl;
        while (searchEl && searchEl !== document.body) {
            let prev = searchEl.previousElementSibling;
            while (prev) {
                // Look for elements likely to be user posts (user-message tag or specific classes)
            ...
            searchEl = searchEl.parentElement; // Move up one level and search again
        }
    ...
    function smartObsidianSave() {
        // 1. Fallback: If focus lost, auto-acquire latest message
        ...
        const sourceUrl = window.location.href; // Link to Gemini thread
        ...
        // --- Fix: Compact Layout Construction (Maintain original spec) ---

        // Header: Remove excess newlines, make compact
        ...
        // Footer: Leave one empty line before hashtags
        let footerInfo = "---\n#Gemini #X_Ops";

        // 2. Character limit and concatenation (LIMIT: 4000)
        const LIMIT = 4000;
        // Length calculation considering separators
        const combinedLength = headerInfo.length + contentBody.length + footerInfo.length;

        let finalBody = "";

        if (combinedLength > LIMIT) {
            // Banner when trimmed (Adjusted for top placement)
            ...
            // ★ Patch: Markdown protection & complete removal of excess newlines (Stable logic)
            ...
            // Remove all trailing whitespace/newlines
            trimmedContent = trimmedContent.replace(/\s+$/, '');

            // Change concatenation order (Header -> Banner -> Body -> Marker -> Footer)
            finalBody = headerInfo + trimBanner + trimmedContent + endMarker + "\n" + footerInfo;

            showToast("⚠️ Content Trimmed", "error");
        } else {
            // Normal: Remove trailing newlines before adding footer
            finalBody = headerInfo + contentBody.replace(/\s+$/, '') + "\n\n" + footerInfo;
        }

        // URI Encode and Launch
        ...
        switch (key) {
            ...
            case 'E': {
                ...
                if (now - lastEPressTime < CONFIG.doubleTapThreshold) {
                    ...
                    lastEPressTime = 0; // Reset after trigger
                } else {
            ...
            case 'Q': {
                ...
                if (now - lastQPressTime < CONFIG.doubleTapThreshold) {
                    navSidebar(-1, e.shiftKey);
                    lastQPressTime = 0; // Reset after trigger
                } else {
